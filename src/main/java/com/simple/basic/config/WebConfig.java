...
public class WebConfig implements WebMvcConfigurer {
    ...
    // 인터셉터로 userAuthHandler를 등록 (ctri + i)
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        registry.addInterceptor( userAuthHandler() ) // 어떤 인터셉터를 등록할 것인지
                ...
                // .addPathPatterns("/user/success"); // 어떤 경로에 인터셉터를 등록할 것인지
                .addPathPatterns("/user/*") // 유저로 시작하는 모든 경로에 등록
                .excludePathPatterns( Arrays.asList("/user/login", "/user/loginForm", "/user/logout") ) // 제외할 경로
                ;
    }
    
    // 만약 종류별로 다양한 인터셉터를 추가할거라면 추가하면 됩니다
